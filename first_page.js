...
function calculateAndDisplaySchedule(totalWordsCount, promptUser = false) {
    ...
    let newWordsPerDayInput = null; // Biến tạm để lưu kết quả từ prompt

    if (promptUser) { // Chỉ hỏi nếu được yêu cầu rõ ràng (khi nhấn 'Điều chỉnh lịch học')
        newWordsPerDayInput = promptForWordsPerDay();
        if (newWordsPerDayInput !== null) { // Người dùng đã cung cấp một số hợp lệ hoặc đã hủy
            currentWordsPerDay = newWordsPerDayInput; // Sử dụng giá trị mới
            // RẤT QUAN TRỌNG: Nếu một số mới hợp lệ được cung cấp (hoặc xác nhận),
            // hãy reset trạng thái hoàn thành của các ngày.
            localStorage.removeItem('completedDays');
            console.log('Đã reset lịch học hoàn thành do điều chỉnh.');
        } else { // Người dùng đã hủy hoặc nhập giá trị không hợp lệ
            // Quay về giá trị hiện có hoặc mặc định nếu prompt bị hủy
            currentWordsPerDay = parseInt(storedWordsPerDay || '1');
            if (currentWordsPerDay === 0 && totalWordsCount > 0) currentWordsPerDay = 1;
            // KHÔNG xóa completedDays nếu người dùng hủy.
        }
    } else if (storedWordsPerDay) { // Không hỏi, chỉ tải lịch hiện có
        currentWordsPerDay = parseInt(storedWordsPerDay);
    } else { // Không có lịch lưu trữ, lần đầu tải
        currentWordsPerDay = 1; // Mặc định
    }

    // Đảm bảo currentWordsPerDay hợp lý nếu có từ
    if (totalWordsCount > 0 && currentWordsPerDay < 1) {
        currentWordsPerDay = 1;
    } else if (totalWordsCount === 0) {
        currentWordsPerDay = 0;
    }
    localStorage.setItem('scheduleWordsPerDay', currentWordsPerDay); // Lưu giá trị (có thể đã cập nhật)

    // Tổng số Day học tính toán: Tổng số từ / Số từ học mỗi ngày (làm tròn lên)
    let daysToStudy = (totalWordsCount > 0 && currentWordsPerDay > 0) ? Math.ceil(totalWordsCount / currentWordsPerDay) : 0;

    dailyScheduleButtonsContainer.innerHTML = ''; // Xóa tất cả các nút hiện có

    if (totalWordsCount === 0) {
        overallScheduleDisplay.textContent = 'Không có từ nào hoặc số từ học mỗi ngày chưa được cài đặt.';
        localStorage.removeItem('completedDays'); // Xóa nếu không có từ nào cả
        words.forEach(word => word.day = undefined); // Nếu không có từ, Day info cũng được khởi tạo
        saveWords();
        return;
    }
    
    // Gán Day học cho từng từ (tất cả các từ được gán lại để duy trì tính nhất quán)
    let wordIndex = 0;
    words.forEach(word => {
        // Nếu currentWordsPerDay là 0, không gán Day (nếu không có từ hoặc cài đặt)
        word.day = (currentWordsPerDay > 0) ? Math.floor(wordIndex / currentWordsPerDay) + 1 : undefined;
        wordIndex++;
    });
    saveWords(); // Lưu mảng words với thông tin day đã gán
    ...
    // Tạo và hiển thị các nút theo ngày
    ...
    for (let day = 1; day <= daysToStudy; day++) {
        ...
        dayBtn.addEventListener('click', () => {
            localStorage.setItem('currentStudyDay', day); // Xác định đây là chế độ học Day
            // Khi nhấn nút Day, hãy xóa tất cả lịch sử học trước đó (luôn bắt đầu từ đầu của Day đó)
            ...
            localStorage.removeItem('isFullStudyCompletedFlag'); // Xóa cờ hoàn thành toàn bộ học
    ...
    // Kiểm tra xem tất cả các Day đã hoàn thành chưa và hiển thị popup
    if (totalWordsCount > 0 && allDaysCompleted && daysToStudy > 0) { 
        alert('Chúc mừng! Bạn đã hoàn thành toàn bộ quá trình học tập!');
        // Khi tất cả quá trình học hoàn thành, hãy xóa completedDays và thông tin học tiếp
        localStorage.removeItem('completedDays'); 
        localStorage.removeItem('lastInterruptedStudyDay');
        localStorage.removeItem('lastInterruptedStudyIndex');
        localStorage.removeItem('lastInterruptedFullStudyIndex');
...
/**
 * Người dùng yêu cầu số từ học mỗi ngày và trả về giá trị hợp lệ.
 */
...
// Google 검색 버튼 이벤트 리스너
if (goToGoogleBtn) {
    goToGoogleBtn.addEventListener('click', () => {
        window.location.href = "https://www.google.com"; // 현재 탭에서 구글로 이동
    });
}
